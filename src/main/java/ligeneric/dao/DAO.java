...
public class DAO {
     ...
     * @throws Exception
     */
    public static <T,K> T findById(Class<T> classe, K id, Connection connexion) throws Exception {
    	T result = null;
    	String request = DaoHelper.getSelectByIdRequest(classe, id);
    	Statement stmt = connexion.createStatement();
    	ResultSet resultSet = stmt.executeQuery(request);
    	if(resultSet.next()) {    		
    		result = DaoHelper.setObject(classe, resultSet,connexion);
    	}
    	return result;
    }
    ...
    public static <T> List<T> findall(Class<T> classe, Connection connexion) throws Exception {
    	...
    	while(resultset.next()) {
    		T element = DaoHelper.setObject(classe, resultset,connexion);
    		liste.add(element);
    	}
    	return liste;
    }
